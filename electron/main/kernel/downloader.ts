...
const logger = Logger.scope('[main] kernel downloader')
...
export class Downloader {
  ...
  private notifyProgress(progress: IKernelUpdaterStatus) {
    getMainWindow()?.webContents.send('kernel-updater-status-change', progress)
    // logger.info(
    //   `Download progress: ${progress.percent.toFixed(2)}%, Speed: ${(progress.bytesPerSecond / 1024 / 1024).toFixed(2)} MB/s`,
    // )
  }
