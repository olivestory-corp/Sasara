...
const logger = Logger.scope('[main] ollama processManager')
...
export class ProcessManager {
  ...
  private async killProcess(pid: string, force = false): Promise<void> {
    try {
      ...
      logger.info(`${force ? 'Force' : 'Normal'} termination of process ${pid}`)
    } catch (error) {
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      logger.error('Error occurred during process cleanup:', error)
      throw error
    }
  }
}
